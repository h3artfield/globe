...
export const unodcAdapter: ReplaySourceAdapter = {
  ...
  async buildEvidenceSnapshot(session) {
    ...
    const rows = filterMetricsBySource(loaded.metrics, "unodc", METRIC_ID);
    ...
    if (included.length === 0) {
      return {
        ...
        confidence: "low",
        limitations: [`No UNODC homicide rows at or before ${cutoff} for ${country}.`],
      };
    }
    ...
    return {
      ...
      limitations: ["UNODC coverage is sparse; verify year availability before high-stakes replay."],
    };
  },
  ...
  async resolve(session) {
    ...
    if (spec.kind !== "metric_compare_years") {
      return resolveMissing("Invalid resolution spec for UNODC adapter");
    }
    ...
    if (!baseline || !holdout) {
      return {
        ...
        confidence: "low",
        limitations: [
          `Missing UNODC rows for baseline year ${baselineYear} and/or resolution year ${resolutionYear}.`,
        ],
      };
    }
