...
export async function loadReplaySession(sessionId: string): Promise<ReplaySession | null> {
  ...
  return readJsonFile<ReplaySession>(filePath).then((session) => ({
    ...
    forecast_mode: session.forecast_mode ?? "historical_replay",
    source_question_id: session.source_question_id ?? null,
    source_market_id: session.source_market_id ?? null,
    external_source: session.external_source ?? null,
    external_source_url: session.external_source_url ?? null,
